...
Fixpoint close_exp_wrt_exp_rec (n1 : nat) (x1 : var) (e1 : exp) {struct e1} : exp :=
  match e1 with
    ...
    | e_save e2 A1 B1=> e_save (close_exp_wrt_exp_rec n1 x1 e2) A1 B1
  end.
...
Fixpoint size_exp (e1 : exp) {struct e1} : nat :=
  match e1 with
    ...
    | e_save e2 A1 B1 => 1 + (size_exp e2) + (size_typ A1) + (size_typ B1)
  end.
...
Inductive degree_exp_wrt_exp : nat -> exp -> Prop :=
  ...
  | degree_wrt_exp_e_save : forall n1 e1 A1 B1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (e_save e1 A1 B1).
...
Inductive lc_set_exp : exp -> Set :=
  ...
  | lc_set_e_save : forall e1 A1 B1,
    lc_set_exp e1 ->
    lc_set_exp (e_save e1 A1 B1).
